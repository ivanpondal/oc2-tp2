...
int kernel_impreciso(float sigm, int m, int radio, unsigned short (*k_impreciso)[m]){	//Editar si se cambia el tipo
	int n = 2*radio + 1;
	double kernel[n][n];
...
//Guardo el arreglo nuevo
	...
	for(int j = 0; j< n; j++){
		for(int i = 0; i< n; i++){
				...
				k_impreciso[i][j] = (unsigned short) a;	//Editar si se cambia el tipo
				}
	}
